...
/**
 * Node.js API for the `audiovisual` app.
 *
 * @module audiovisual
 */
        ...
        new jsmediatags.Reader(filePath)
            ...
            .read({
                ...
                onError: function(error) {
                    reject(new Error(
                        `Failed to parse tags for '${filePath}':\n${error.info}`
                    ));
                }
            });
    });
 ...
 *
 * @param {module:audiovisual~Options} opts - Configuration.
...
function scanFiles(opts) {
    ...
    /**
     * Adds the given directory's contents to the file list.
     *
    ...
    /**
     * Adds the given file to the file list, scanning for tags as appropriate.
     *
    ...
    function addFile(filePath) {
        const url = path.posix.join(
            filesDirURL,
            path.relative(filesDir, filePath).replace(/\\/g, '/')
        );
        tick && tick(false, url);
    ...
    /**
     * Adds the given path to the file list, calling `addFile` or `addDir` as
     * appropriate.
     *
     * @param {string} filePath - Path to the file.
     */
 ...
 *
 * @param {module:audiovisual~Options} opts - Configuration options.
